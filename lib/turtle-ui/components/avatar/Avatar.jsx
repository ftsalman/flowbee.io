...
export const Avatar = ({
  ...
  if (imgSrc) {
    return (
      <div
        className={cn("tui-avatar overflow-hidden", className)}
        {...defaultProps}
      >
        <img
          src={imgSrc}
          ...
          alt="avatar"
          onError={(e) => {
            e.target.src = "/images/profile-default.png"; // fallback image
          }}
          {...imgProps}
          className={cn("tui-avatar-img object-cover w-full h-full", imgProps?.className)}
        />
      </div>
    );
  }
