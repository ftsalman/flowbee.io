...
export const ArticleView = ({
  ...
  const [activeToc, setActiveToc] = useState("we-are-a-meta-business-partner");
  const [dynamicToc, setDynamicToc] = useState([]);
  const [processedContent, setProcessedContent] = useState("");
  const navigate = useNavigate();

  useEffect(() => {
    if (activeModule?.isDynamic && activeModule?.content) {
      const parser = new DOMParser();
      const doc = parser.parseFromString(activeModule.content, "text/html");
      ...
      if (toc.length > 0) setActiveToc(toc[0].id);
    } else {
      setDynamicToc([]);
      setProcessedContent("");
      setActiveToc("we-are-a-meta-business-partner");
    }
  }, [activeModule]);
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-6 flex flex-col lg:flex-row gap-8 items-start">
      <style>{`
      ...
      <aside className="w-full lg:w-[280px] shrink-0 lg:border-r border-gray-100 lg:pr-4 lg:sticky lg:top-24 lg:h-[calc(100vh-120px)] lg:overflow-y-auto lg:overflow-x-hidden custom-scroll order-2 lg:order-1">
              ...
              return (
                <div key={cat.id} className="flex flex-col">
                  <div
                    ...
                    className={`flex items-start gap-2 py-2 px-2 rounded-lg cursor-pointer transition-colors ${isActiveCat ? "text-gray-900 font-bold" : "text-gray-600 hover:bg-gray-50"}`}
                  >
                    <span className="text-gray-400 mt-0.5 shrink-0">
                      <cat.icon size={16} />
                    </span>
                    <span className="text-[15px] flex-1 leading-snug">{cat.label}</span>
                    <span className="ml-auto mt-0.5 text-[11px] font-bold bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full shrink-0">
                      {modules.length}
                    </span>
                  </div>
                      ...
                      {modules.map((mod, idx) => {
                        ...
                        return (
                          <div
                            ...
                            className={`text-left py-2 px-3 rounded-md text-[14px] cursor-pointer transition-colors ${
                              isActiveMod
                                ? "bg-[#FFD400]/20 text-gray-900 font-semibold border-l-4 border-[#FFD400] -ml-[18px] pl-[30px]"
      ...
      {/* Main Content Area */}
      <main className="flex-1 lg:px-8 pb-10 lg:pb-20 flex justify-center order-1 lg:order-2 w-full overflow-x-hidden">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 overflow-hidden shrink-0 border border-gray-100">
                {activeModule?.creatorImage ? (
                  <img
                    src={activeModule.creatorImage}
                    alt={activeModule?.creatorName || "Creator"}
                ...
                <span className="text-[13px] text-gray-500 flex items-center gap-2">
                  <span>
                    {activeModule?.createdDate
                      ? new Date(activeModule.createdDate).toLocaleDateString(
                          "en-US",
              ...
              {activeModule?.isDynamic ? (
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: processedContent || activeModule.content,
                  }}
                />
              ) : (
                <>
                  ...
                  <p>
                    This is a detailed documentation page for the{" "}
                    <strong>{activeModule?.title}</strong> module. In a
                    real-world scenario, this content would be fetched from a
                  ...
                  <pre className="bg-[#0f172a] text-white p-6 rounded-2xl overflow-x-auto text-sm shadow-xl font-mono leading-relaxed my-6">
                    <span className="text-pink-400">import</span> flowbee{" "}
                    <span className="text-pink-400">from</span> 'flowbee-sdk';
                    {"\n\n"}
                    <span className="text-gray-400">// Initialize client</span>
                    {"\n"}
                    <span className="text-blue-300">const</span> client ={" "}
                    <span className="text-pink-400">new</span> flowbee.Client();
                    {"\n\n"}
                    <span className="text-blue-300">const</span> response ={" "}
                    <span className="text-pink-400">await</span>{" "}
                    client.modules.invoke({"{"}
                    {"\n"}
                    {"  "}name:{" "}
                    <span className="text-green-300">
                      "{activeModule?.title?.replace(/\s+/g, "_").toLowerCase()}
                      "
                    </span>
                    ,{"\n"}
                  ...
                  <p>
                    Ensure you handle the asynchronous responses properly in
                    your application lifecycle. If you encounter any issues,
                    please submit a support ticket using the form below.
                  </p>

                  <h3 id="we-are-a-meta-business-partner">
                    We Are a Meta Business Partner
                  </h3>
                  <p>
                    To get started, navigate to your dashboard and create a new
                    access token. Make sure you store it securely, as it will
                    only be displayed once.
                  </p>

                  <h3 id="the-problem-businesses-face">
                    The Problem Businesses Face
                  </h3>
                  <p>
                    Next, configure your webhook endpoint to receive real-time
                    updates. Your endpoint must be capable of responding with a
                    200 OK within 3 seconds to avoid timeouts.
                  </p>

                  <h3 id="how-flowbee-solves-it">How Flowbee Solves It</h3>
                  <p>
                    Our API supports batch operations. You can send up to 1,000
                    records in a single payload. Ensure your payload follows the
                    JSON schema defined in our API reference.
                  </p>

                  <h3 id="broadcast-messaging">Broadcast Messaging</h3>
                  <p>
                    Additionally, rate limits apply. You can make up to 100
                    requests per minute on the standard plan. For higher limits,
                  ...
                  <p>
                    ...
                    API key. If the issue persists, try rotating your key from
                    the developer portal.
                  </p>

                  <h3 id="team-inbox">Team Inbox</h3>
                  <p>
                    For 429 Too Many Requests, implement exponential backoff in
                    your retry logic to prevent being temporarily banned.
                  </p>

                  <h3 id="booking-&-appointment">Booking & Appointment</h3>
                  <p>
                    Join our developer community on Discord or our community
                    forums to share your use cases, ask questions, and
                    collaborate with other developers building with Flowbee.
                  </p>
                </>
              )}
            </div>
      ...
      <aside className="w-full lg:w-[270px] shrink-0 lg:pl-6 lg:sticky lg:top-24 lg:h-[calc(100vh-120px)] lg:overflow-y-auto lg:overflow-x-hidden custom-scroll pb-10 order-3 hidden xl:block">
        <div className="sticky top-0 space-y-10">
          {/* Table of Contents */}
          <div>
            <h3 className="text-[15px] font-bold text-gray-500 mb-6 uppercase tracking-wider">
              Table of Contents
            </h3>
            <div className="relative border-l-2 border-gray-100 flex flex-col gap-5 text-[14px]">
              {(activeModule?.isDynamic
                ...
                : [
                    {
                      id: "we-are-a-meta-business-partner",
                      text: "We Are a Meta Business Partner",
                    },
                    {
                      id: "the-problem-businesses-face",
                      text: "The Problem Businesses Face",
                    },
                    {
                      id: "how-flowbee-solves-it",
                      text: "How Flowbee Solves It",
                    },
                    { id: "broadcast-messaging", text: "Broadcast Messaging" },
                    { id: "chatbot-automation", text: "Chatbot Automation" },
                    { id: "team-inbox", text: "Team Inbox" },
                    {
          ...
          {/* Share Box */}
          <div className="bg-[#F8F9FA] rounded-[16px] p-2">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 text-[14px] font-bold bg-white border border-gray-200 px-4 py-3 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors shadow-sm"
            >
