...
export const PricingCards = ({ selectedRegion, billingCycle }) => {
  const getPriceData = (plan) => {
    ...
    const baseData = isUSD
      ? pricingTable.AED
      : pricingTable[selectedRegion.code];
    const rate = isUSD ? 0.27 : 1;
    const data = baseData[plan];

    return {
      monthly: data.monthly
        ? (data.monthly * rate).toFixed(isUSD ? 2 : 0)
        : undefined,
      quarterly: data.quarterly
        ? (data.quarterly * rate).toFixed(isUSD ? 2 : 0)
        : undefined,
      halfYearly: data.halfYearly
        ? (data.halfYearly * rate).toFixed(isUSD ? 2 : 0)
        : undefined,
    ...
    <section className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 py-12 lg:py-24">
      {["starter", "growth", "pro"].map((plan) => {
        ...
        return (
          ...
          >
            {isGrowth && (
              <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-[#FFDD2D] text-[#111] text-[9px] lg:text-[10px] font-black px-6 py-2 rounded-full uppercase tracking-[0.2em] shadow-xl">
                Best Value
              </div>
            )}
            ...
            <div className="mb-8 lg:mb-10 text-left">
              <h3
                className={`text-md lg:text-xl font-black mb-4 uppercase flex items-center gap-2 ${isGrowth ? "text-[#FFDD2D]" : "text-[#111]"}`}
              >
              ...
              {billingCycle === "yearly" && (
                <p className="mt-4 text-[9px] lg:text-[10px] font-black text-[#25D366] uppercase tracking-widest bg-[#25D366]/10 inline-block px-3 py-1 rounded-lg">
                  🎁 Save {selectedRegion.symbol}
                  {data.save} Yearly
                </p>
              )}
            </div>
                ...
                {[
                  ...
                  .map((f, i) => (
                    <div key={i} className="flex gap-3">
                      <FiCheck
                        className={`shrink-0 mt-0.5 ${isGrowth ? "text-[#FFDD2D]" : "text-[#25D366]"}`}
                      />
                      <span
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className={`w-full py-4 lg:py-5 rounded-2xl font-black text-[10px] lg:text-xs uppercase tracking-widest text-center shadow-xl hover:scale-105 transition-all ${isGrowth ? "bg-[#FFDD2D] text-[#111]" : "bg-[#111] text-white"}`}
            >
              Get Started
            </a>
          </motion.div>
