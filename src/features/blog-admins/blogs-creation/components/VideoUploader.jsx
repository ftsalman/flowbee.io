...
export const VideoUploader = ({ 
  ...
  const handleUpload = async (file) => {
    ...
    try {
      const url = await uploadToCloudinary(file, "video", (pct) => setProgress(pct));
      onChange(url);
    } catch (err) {
        ...
        {isUploading ? (
          ...
          <div className="w-full h-full relative group min-h-[120px]">
            ...
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center pointer-events-none">
              <span className="text-white text-xs font-bold bg-black/40 px-3 py-1.5 rounded-full pointer-events-auto cursor-pointer" onClick={(e) => { e.stopPropagation(); inputRef.current?.click(); }}>
                Change Video
              </span>
            </div>
          </div>
